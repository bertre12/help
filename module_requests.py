...
POST - метод используется для отправки/создания данных на сервер.
...
# rprint(f'{"Content-Type"} - {response.headers["Content-Type"]}')  # Определение какой тип контента в ответе сервера.
#
...
"""Поиск по запросу. (api - взят для примера)"""


# # Прямое указание параметров поиска через ? и &.
#
# url = 'https://randomuser.me/api/?gender=male&nat=de'
# response = requests.get(url, stream=True).json()
#
# rprint(response)

# # Указание параметров поиска через переменную.
#
# query_params = {"gender": "male", "nat": "de"}  # Переменная параметров поиска.
# url = 'https://randomuser.me/api/'
# response = requests.get(url, stream=True, params=query_params).json()
#
# rprint(response)
